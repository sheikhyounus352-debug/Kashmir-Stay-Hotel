...
import { VerifiedHotelKnowledge, AuthSession, AgentAuthSession } from '../types';
...
interface NavbarProps {
  ...
  activeView: 'receptionist' | 'management' | 'agent-portal';
  onChangeView: (view: 'receptionist' | 'management' | 'agent-portal') => void;
  authSession: AuthSession | null;
  onOpenAdminLogin: () => void;
  onAdminLogout: () => void;
  agentSession: AgentAuthSession | null;
      ...
      <div className="bg-[#08221a] px-4 py-1.5 text-xs text-emerald-200/90 border-b border-emerald-950 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">
            Strict Zero-Assumption Mode: AI Receptionist & Agent Portal operate strictly on verified hotel records.
          </span>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs">
          ...
          {agentSession ? (
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-amber-950/80 text-amber-300 border border-amber-800 text-[11px] font-bold">
                <Briefcase className="w-3 h-3 text-amber-400" />
                <span>Agent: {agentSession.agent.agencyName} ({agentSession.agent.commissionPercentage}%)</span>
              </span>
              <button
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-emerald-300 hover:text-emerald-100 transition-colors cursor-pointer text-[11px]"
            >
              <Briefcase className="w-3 h-3 text-amber-400" />
              <span>Travel Agent Login (B2B)</span>
            </button>
          )}

          <div className="h-3 w-[1px] bg-emerald-900/80"></div>
          ...
          {authSession ? (
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-emerald-900/90 text-emerald-300 border border-emerald-700 text-[11px] font-bold">
                ...
                <span>Admin: {authSession.user.username}</span>
              </span>
              <button
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-amber-300 hover:text-amber-200 transition-colors cursor-pointer text-[11px]"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center bg-[#071f17] p-1 rounded-xl border border-emerald-900/70 shadow-inner">
          ...
          >
            ...
            <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-amber-950/80 text-amber-300 border border-amber-800">
              B2B
            </span>
          </button>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {activeView === 'agent-portal' ? (
            <button
              onClick={() => onChangeView('receptionist')}
              className="px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-semibold flex items-center gap-2 transition-all shadow-md bg-emerald-800/80 hover:bg-emerald-700 text-emerald-100 border border-emerald-600 cursor-pointer"
            >
              <MessageSquareQuote className="w-4 h-4 text-amber-400" />
              <span>Back to Receptionist</span>
            </button>
          ) : activeView === 'management' ? (
            <button
              onClick={() => onChangeView('receptionist')}
            ...
            <button
              onClick={handleAgentClick}
              className="px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-semibold flex items-center gap-2 transition-all shadow-md bg-amber-500 hover:bg-amber-400 text-stone-950 font-bold border border-amber-600 cursor-pointer"
            >
              <Briefcase className="w-4 h-4 text-stone-950" />
              <span>{agentSession ? 'Agent Dashboard' : 'Travel Agent Portal'}</span>
            </button>
          )}
        </div>
      </div>
